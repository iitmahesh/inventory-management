...
export class PurchaseDetailsPage implements OnInit {
  ...
  updateFilter(event) {
    ...
    const temp = this.temp.filter((d) => {
      console.log(d);
      return d.productDesc.toLowerCase().indexOf(val) !== -1 ||
             d.productCode.toLowerCase().indexOf(val) !== -1  ||
             d.invoiceNumber.toString().indexOf(val) !== -1  ||
             d.purchaseDate.toString().indexOf(val) !== -1  ||
             d.qtyWithQtyType.toLowerCase().indexOf(val) !== -1  ||
             d.rate.toString().indexOf(val) !== -1  ||
             d.totalAmount.toString().indexOf(val) !== -1  ||
             d.payment.toString().indexOf(val) !== -1  ||
